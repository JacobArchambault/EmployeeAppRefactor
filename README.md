...
### 1.1 Clean up unused code immediately
...
## 2 Encapsulation and object cohesion
...
### 2.2 Keep methods with the classes they act on.
...
### 2.5 Method names should be a noun or a verb, nothing more
...
### 3.3 Asynchronous applications and multithreaded applications
...
The main theoretical problem with inheritance is that the assimilation of object-oriented programming objects to real-world ones is misleading. In practice, very little code in object oriented programming languages actually adhere to the ideal of having objects reflect real-world entities: such an endeavor is even antithetical to theoretical virtues like object cohesion and loose coupling, and even classes that fail to exhibit these virtues are far more likely to name roles (e.g. builder) than types (e.g. dog). Most strikingly, where real-world objects act primarily on other objects, object-oriented programming objects primarily [act on themselves](https://jacobarchambault.com/2019/05/10/object-oriented-programming-objects-arent-objects/). 

### 4.3 Composition via interface implementations and dependency injection
From a computational standpoint, the main import of inheritance is that it makes code in one object accessible to other objects via a many-to-one relationship. But such a relationship can be obtained in a cleaner way and more flexible way by dependency injection, i.e. by passing the would-be parent object as an argument into the would-be child object's constructor and assigning it therein to an internal fields. In this way, the would-be parent object's functionality remains accessible to its would-be child class without negatively affecting that class' cohesion. In addition, if the constructor is written to require an interface instead of a particular concrete class, it becomes possible to pass in alternate implementations, achieving greater versatility by decoupling the would-be child from a particular class. 
