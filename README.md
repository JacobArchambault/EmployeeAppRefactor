...
The main problem with inheritance arises is that the assimilation of object-oriented programming objects to real-world ones is misleading. In practice, very little code in object oriented programming languages actually adhere to the ideal of having objects reflect real-world entities: such an endeavor is even antithetical to theoretical virtues like object cohesion and loose coupling, and even classes that fail to exhibit these virtues are far more likely to name roles (e.g. builder) than types (e.g. dog). Most strikingly, where real-world objects act primarily on other objects, object-oriented programming objects primarily act on themselves. 
